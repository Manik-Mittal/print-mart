...
import CategoryForm from '../../Layout/Form/CategoryForm';
...
const CreateCategory = () => {
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      const { data } = await axios.put(
        `http://localhost:8000/api/v1/vendor/update-category/${selected._id}`,
        { name: updatedName }
      );
      if (data?.success) {
  ...
  const handleDelete = async (pId) => {
    try {
      const { data } = await axios.delete(
        `http://localhost:8000/api/v1/vendor/delete-category/${pId}`
      );
      if (data.success) {
  ...
  return (<Layout title={"Dashboard - Create Category"}>
    <div className="container-fluid m-3 p-3 dashboard" style={{
      backgroundImage: `url(/purple.gif)`, // Using the purple.gif file from the public folder
